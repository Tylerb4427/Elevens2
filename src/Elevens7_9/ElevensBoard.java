...
public class ElevensBoard extends Board {
     ...
     *
     * @param sC selectedCards the list of the indices of the selected cards.
     * @return true if the selected cards form a valid group for removal; false
     * otherwise.
     */
    @Override
    public boolean isLegal(List<Integer> sC) {
        return ((sC.size()==2&&(cardAt(sC.get(0)).pointValue()+cardAt(sC.get(1)).pointValue())==11)||(hasA("king",sC)&&hasA("queen",sC)&&hasA("jack",sC)));
    }
    
    public boolean hasA( String str,List<Integer> sC){
        for(int i=0;i<3&&i<sC.size();i++)
            if(cardAt(sC.get(i)).rank().equals(str))
                return true;
        return false;
    }
    public boolean hasA( String str){
        for(int i=0;i<9;i++)
            if(cardAt(i).rank().equals(str))
                return true;
        return false;
    ...
    private boolean containsJQK() {
        return (hasA("king")&&hasA("queen")&&hasA("jack"));
    }
